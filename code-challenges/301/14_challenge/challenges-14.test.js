...
let biggerThanLuke = (arr) => {
  ...
  arr.forEach( character => {
    if (parseInt(character.mass) > 77){
      newString += ` - ${character.name}`;
    }
  });
  return newString.slice(3);
};
...
const detectTicTacToeWin = (board) => {
  ...
  let dia2 = [board[2][0], board[1][1], board[0][2]];

  let newArr = [];

  function helper(rowOrColumn){
    console.log(rowOrColumn);
    if (rowOrColumn === ['X', 'X', 'X']) {
      console.log('success');
      newArr.push('winner');
    }else {
      newArr.push('loser');
